...
export default function Countdown({ target, label = '마감' }: { target: string; label?: string }) {
  ...
  if (!remaining) {
    return <span className="text-xs text-gray-400 font-medium">마감됨</span>
  }

  const isUrgent = remaining.days === 0

  return (
    <span className={`text-xs font-mono font-bold tracking-wide ${isUrgent ? 'text-red-500' : 'text-blue-600'}`}>
      {label && <span className="font-sans font-normal text-gray-400 mr-1">{label}</span>}
      {remaining.days > 0
        ? `D-${remaining.days}`
