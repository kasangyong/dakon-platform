import type { Hackathon, HackathonDetail, Leaderboard, Team, CommunityVoteStat } from '@/types'
...
export const seedHackathonDetails: HackathonDetail[] = [
  ...
  },
  {
    ...
    sections: {
      overview: {
        summary: '바이브 코딩 워크플로우를 혁신할 AI 아이디어를 제안하는 공모전입니다.',
        teamPolicy: { allowSolo: true, maxTeamSize: 4 },
      },
      info: {
        notice: [
          '아이디어 기반 공모전으로 실제 구현 없이 기획서만 제출합니다.',
          '심사는 실현 가능성, 독창성, 임팩트 기준으로 진행됩니다.',
        ],
        links: { rules: '#', faq: '#' },
      },
      eval: {
        metricName: 'IdeaScore',
        description: '실현가능성 30% + 독창성 40% + 임팩트 30%',
        scoreSource: 'vote',
        scoreDisplay: {
          label: '심사 점수',
          breakdown: [
            { key: 'feasibility', label: '실현가능성', weightPercent: 30 },
            { key: 'originality', label: '독창성', weightPercent: 40 },
            { key: 'impact', label: '임팩트', weightPercent: 30 },
          ],
        },
      },
      schedule: {
        timezone: 'Asia/Seoul',
        milestones: [
          { name: '접수 시작', at: '2026-02-10T10:00:00+09:00' },
      ...
      submit: {
        ...
        guide: ['아이디어 기획서를 텍스트 또는 URL로 제출합니다.'],
        submissionItems: [
...
export const seedLeaderboards: Leaderboard[] = [
  {
    ...
    entries: [
      ...
      { rank: 3, teamName: 'ModelMinimizers', score: 0.6891, submittedAt: '2026-02-25T08:11:00+09:00' },
      { rank: 4, teamName: 'LightWeightChamps', score: 0.6754, submittedAt: '2026-02-24T18:30:00+09:00' },
      { rank: 5, teamName: 'NeuralNinjas', score: 0.6601, submittedAt: '2026-02-25T09:55:00+09:00' },
    ],
  },
  {
...
// Community vote baselines (total stars + count per team per hackathon)
// Used to show realistic community averages even before the user votes
export const seedCommunityVotes: Record<string, Record<string, { total: number; count: number }>> = {
  'daker-handover-2026-03': {
    '404found':  { total: 347, count: 82 },  // avg ≈ 4.23
    'LGTM':      { total: 302, count: 75 },  // avg ≈ 4.03
    'VibeDevs':  { total: 218, count: 60 },  // avg ≈ 3.63
  },
  'aimers-8-model-lite': {
    'Team Alpha':         { total: 285, count: 68 },  // avg ≈ 4.19
    'Team Gamma':         { total: 261, count: 65 },  // avg ≈ 4.02
    'ModelMinimizers':    { total: 231, count: 60 },  // avg ≈ 3.85
    'LightWeightChamps':  { total: 198, count: 55 },  // avg ≈ 3.60
    'NeuralNinjas':       { total: 180, count: 52 },  // avg ≈ 3.46
  },
}
...
export const seedTeams: Team[] = [
  ...
  },
  {
    ...
    intro: '바이브 코딩으로 빠른 프로토타이핑을 목표로 합니다.',
