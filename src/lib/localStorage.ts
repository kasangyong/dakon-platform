...
const KEYS = {
  ...
  notes: 'dakon_notes',
  votes: 'dakon_votes',
  communityVotes: 'dakon_community_votes',
  myCheers: 'dakon_my_cheers',
} as const
...
export function initSeed() {
  ...
  if (!localStorage.getItem(KEYS.votes)) {
    set(KEYS.votes, {} as Record<string, MyVote>)
  }
  if (!localStorage.getItem(KEYS.communityVotes)) {
    set(KEYS.communityVotes, seedCommunityVotes)
  }
}
...
export function updateTeam(teamCode: string, patch: Partial<Omit<Team, 'teamCode' | 'hackathonSlug' | 'createdAt'>>): void {
  const teams = getTeams()
...
export function deleteTeam(teamCode: string): void {
  const teams = getTeams()
  set(KEYS.teams, teams.filter((t) => t.teamCode !== teamCode))
}
...
export function getMyCheers(): string[] {
  return get(KEYS.myCheers, [])
}

export function toggleCheer(teamCode: string): { count: number; cheered: boolean } {
  const cheers = getCheers()
  const myCheers = getMyCheers()
  const alreadyCheered = myCheers.includes(teamCode)

  if (alreadyCheered) {
    const next = Math.max(0, (cheers[teamCode] ?? 1) - 1)
    set(KEYS.cheers, { ...cheers, [teamCode]: next })
    set(KEYS.myCheers, myCheers.filter((c) => c !== teamCode))
    return { count: next, cheered: false }
  } else {
    const next = (cheers[teamCode] ?? 0) + 1
    set(KEYS.cheers, { ...cheers, [teamCode]: next })
    set(KEYS.myCheers, [...myCheers, teamCode])
    return { count: next, cheered: true }
  }
}

// ─── Team status ──────────────────────────────────────────────────────────────

export function updateTeamOpen(teamCode: string, isOpen: boolean): void {
  const teams = getTeams()
  const idx = teams.findIndex((t) => t.teamCode === teamCode)
  if (idx < 0) return
  teams[idx] = { ...teams[idx], isOpen }
  set(KEYS.teams, teams)
}
...
export function getMyVote(slug: string): MyVote | null {
  const all = get<Record<string, MyVote>>(KEYS.votes, {})
  return all[slug] ?? null
}

export function hasVoted(slug: string): boolean {
  return !!getMyVote(slug)
}

export function castVote(slug: string, ratings: Record<string, number>): void {
  // Save user's vote
  const all = get<Record<string, MyVote>>(KEYS.votes, {})
  const prev = all[slug]
  all[slug] = { hackathonSlug: slug, ratings, votedAt: new Date().toISOString() }
  set(KEYS.votes, all)

  // Update community vote aggregates
  const community = get<Record<string, Record<string, { total: number; count: number }>>>(
    KEYS.communityVotes,
    seedCommunityVotes,
  )
  const hackVotes = community[slug] ?? {}

  for (const [teamName, stars] of Object.entries(ratings)) {
    const existing = hackVotes[teamName] ?? { total: 0, count: 0 }
    // Remove previous vote if changing
    if (prev?.ratings[teamName] !== undefined) {
      existing.total = existing.total - prev.ratings[teamName] + stars
    } else {
      existing.total += stars
      existing.count += 1
    }
    hackVotes[teamName] = existing
  }

  community[slug] = hackVotes
  set(KEYS.communityVotes, community)
}

export function getCommunityVoteStats(slug: string): CommunityVoteStat[] {
  const community = get<Record<string, Record<string, { total: number; count: number }>>>(
    KEYS.communityVotes,
    seedCommunityVotes,
  )
  const hackVotes = community[slug] ?? {}
  return Object.entries(hackVotes)
    .map(([teamName, { total, count }]) => ({
      teamName,
      avgStars: count > 0 ? total / count : 0,
      count,
    }))
